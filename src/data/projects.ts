...
export const projects: Project[] = [
    {
        ...
        role: `Technical orchestration for Oatly's location finder rebuild. Using Claude Code, I coordinated specialized AI agents for feature development, ensured responsive design implementation, managed content structure in Storyblok, and validated Next.js deployment architecture on Vercel.`,
        year: '2026',
        image: '/uploads/oatfinder-demo.gif',
        images: [
            '/uploads/oatfinder-demo.gif',
            '/uploads/oatfinder-mobile.jpg',
        ],
    ...
    },
    {
        ...
        description:
            'n8n automation workflow connecting Shopify orders to Sendify shipping labels and Bokio accounting',
        challenge: `Running an Italian food e-commerce business (Cuppino) required manual operations across multiple systems: every Shopify order needed shipping label creation in Sendify, VAT calculation, revenue categorization across multiple accounts (6% food, 12% old rates, 25% shipping), and manual entry into Bokio accounting. This consumed hours weekly and introduced human error risk. Additionally, invoices arriving via email needed manual download, upload to accounting, and backup to cloud storage—creating a second time-consuming bottleneck.`,
        solution: `Built two automated workflows using n8n: (1) Shopify-to-Sendify-to-Bokio order automation that fetches paid orders, automatically creates shipping labels via Sendify API, calculates Swedish VAT breakdown, sends formatted entries to Telegram for approval, creates journal entries in Bokio on acceptance, and updates confirmation messages. The system handles multiple VAT rates (6%, 12%, 25%), shipping calculations, and discount codes automatically. (2) Invoice processor that monitors ProtonMail inbox, extracts PDF attachments, uploads to Bokio API, backs up to Nextcloud organized by year/month, and marks processed. Both run on n8n Cloud with daily schedules, comprehensive error handling, and production use since April 2026.`,
        role: `Designed and implemented both automation workflows: architected the n8n node structure, integrated Shopify Admin API with OAuth authentication, connected Sendify API for automatic shipping label generation, built Telegram bot interface with interactive approval buttons, created accounting logic for Swedish VAT rules (including 2026 rate changes), integrated Bokio API for journal entry creation, developed ProtonMail IMAP monitoring with ProtonMail Bridge, implemented Nextcloud WebDAV backup system, and deployed to production with cron scheduling and monitoring.`,
        year: '2026',
        image: '/uploads/n8n-white.jpg',
        images: ['/uploads/n8n-white.jpg', '/uploads/n8n-workflow.png'],
        link: null,
        moreInfo: null,
        videoUrl: null,
